...
class InsightGenerator:
    ...
    def generate_insights(cls, filters: Dict[str, Any]) -> List[Dict[str, Any]]:
        ...
        try:
            sim_data = SimulationService.simulate_scenario({})
        ...
        # 1. SLA Insights
        sla_comp = sla_summary.get("predicted_sla_compliance", 100.0)
        insights.append({
            "category": "SLA & Compliance",
            "icon": "fa-hourglass-half",
            "color": "#f59e0b" if sla_comp < 85 else "#10b981",
            "title": f"Predicted SLA Compliance at {sla_comp}%",
            "detail": f"Active cohort shows {sla_summary.get('high_risk_shipments', 0)} shipments flag as High/Critical risk, introducing potential delay overheads."
        })

        # 2. Cost Insights
        savings = sim_summary.get("projected_annual_savings", 0.0)
        insights.append({
            "category": "Cost Optimization",
            "icon": "fa-calculator",
            "color": "#10b981",
            "title": f"Projected Cost Savings of ${savings:,.2f}",
            "detail": f"What-If optimization model suggests that driver reallocations and lane consolidating yields up to {sim_summary.get('projected_savings_pct', 0.0)}% overhead reduction."
        })

        # 3. Reverse Logistics Insights
        recovery = rev_an.get("recovery_rate", 0.0)
        insights.append({
            "category": "Reverse Logistics",
            "icon": "fa-rotate-left",
            "color": "#06b6d4",
            "title": f"Asset Recovery Rate is {recovery}%",
            "detail": f"Refurbishment and recycling loops successfully captured ${rev_an.get('recovered_value', 0.0):,.2f} in scrap and parts values this cycle."
        })

        # 4. Route Congestion Bottleneck
        insights.append({
            "category": "Network Bottlenecks",
            "icon": "fa-triangle-exclamation",
            "color": "#ef4444",
            "title": "Corridor HUB-E → HUB-B High Delay",
            "detail": "Historical transit records indicate persistent SLA misses on the long-haul lane. Rerouting via A* is advised."
        })
